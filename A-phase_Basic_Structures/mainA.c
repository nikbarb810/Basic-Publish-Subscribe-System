...
{
    struct Group G[64];
    struct SubInfo *S = SL_Initialize();
	FILE *fin = NULL;
	char buff[BUFFER_SIZE], event='\0';


    struct SubInfo  *ar[64];
	...
	while (fgets(buff, BUFFER_SIZE, fin))
	{
		...
		case 'I':
		{
			...
			if (Insert_Info(itm,iId,gids_arr,num_of_gids,G,&S)==0)
			{
				DPRINT("%c <%d> <%d> DONE\n",buff[0], itm,iId);
			}
			else
		...
		case 'S':
		{
            ...
            gids_arr= event_args_arr+2;
            num_of_gids-=2;
            if (Subscriber_Registration(sTM,sId,gids_arr,num_of_gids,G,&S)==0)
            {
                DPRINT("%c <%d> <%d> DONE\n", buff[0],sTM,sId);
            }
            else
		...
		case 'D':
		{
			int sId;
			sscanf(buff, "%c %d", &event, &sId);
            if (Delete_Subscriber(&G,&S,sId)==0)
			{
				DPRINT("%c <%d> DONE\n", event, sId);
			}
			else
