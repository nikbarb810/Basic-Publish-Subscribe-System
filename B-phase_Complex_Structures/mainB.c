 ...
 *
 * @brief   Main function for the needs of CS-240 A-phase_Basic_Structures 2022.Prof. Panagiota Fatourou.
...
int main(int argc, char **argv)
{
    ...
    struct SubInfo **T = HT_Initialize(universalHashingNumber,hashTableSize);


	/* Initializations */
	initialize(hashTableSize, universalHashingNumber,G,T);
	...
	while (fgets(buff, BUFFER_SIZE, fin))
	{
		...
		case 'I':
		{
            ...
            num_of_gids-=2;
			if (Insert_Info(itm,iId,gids_arr,num_of_gids,G)==0)
			{
				DPRINT("%c <%d> <%d> DONE\n",buff[0], itm,iId);
			}
			else
		...
		case 'S':
		{
            ...
            gids_arr= event_args_arr+2;
            num_of_gids-=2;
            if (Subscriber_Registration(sTM,sId,gids_arr,num_of_gids,G,T)==0)
            {
                DPRINT("%c <%d> <%d> DONE\n", buff[0],sTM,sId);
            }
            else
		...
		case 'D':
		{
			int sId;
			sscanf(buff, "%c %d", &event, &sId);
            if (Delete_Subscriber(sId,G,T)==0)
			{
				DPRINT("%c <%d> DONE\n", event, sId);
			}
			else
